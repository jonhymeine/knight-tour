...
int main(int argc, char *argv[])
{
  ...
  bool found_closed_tour = false;

  // for (int i = 0; i < BOARD_SIZE; ++i)
  // {
  //   if (found_closed_tour)
  //     break;
  ...
  // pair<int, int> initial_position = make_pair(i, j);
  // pair<int, int> knight_position = initial_position;

  // Reset state variables for each starting position
  ...
  while (step <= max_steps)
  {
    ...
    // Check if tour is complete AND can close the loop
    if (step == max_steps + 1)
    {
      if (!isReachable(knight_position, initial_position))
      {
        // Tour is complete but not closed - force backtracking
        cout << "Tour complete but not closed. Backtracking to find closed tour...\n";
        step--; // Treat as if we need to backtrack
        backtracks++;

        if (move_history.empty())
        {
          cout << "Cannot find a closed tour from this starting position.\n";
          break;
        }
  ...
  if (isReachable(knight_position, initial_position))
  {
    ...
    cout << "Starting position: (" << initial_position.first << ", " << initial_position.second << ")\n";
    found_closed_tour = true;
  }
  else
