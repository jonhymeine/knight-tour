...
bool goToNextPosition(pair<int, int> &current_position, vector<vector<Direction>> &visited_squares, Direction ignored_direction)
{
...
bool goToNextPositionHeuristic(pair<int, int> &current_position, vector<vector<Direction>> &visited_squares, const vector<vector<int>> &possible_moves_board, Direction ignored_direction)
{
  auto [row, col] = current_position;
  ...
  current_position = make_pair(best_row, best_col);
...
void mapPossibleMoves(vector<vector<int>> &board)
{
  const int delta_row[8] = {-2, -1, 1, 2, 2, 1, -1, -2};
  const int delta_col[8] = {1, 2, 2, 1, -1, -2, -2, -1};
